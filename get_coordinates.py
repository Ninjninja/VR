...
class FetchFrame2:
    def __init__(self, file="../datasets/Elie_room_data/joints.pkl"):
        file = open(file, 'rb')
        self. frame = pkl.load(file)
        file.close()
        self.count = 0
    ...
    def __next__(self):
        self.count = 1
        frame0 = self.frame[self.count]
        fingers = ['Thumb', 'Index', 'Middle', 'Ring', 'Pinky']
        bones = ['_Distal_next', '_Intermediate_next', '_Proximal_next', '_Metacarpal_next', '_Metacarpal_prev']
        joints = []
        points = []

        for finger in fingers:
            for bone in bones:
                points.append(self.str2coordinates(frame0[finger + bone]))
            joints.append(points)
            points = []
        joints.append(self.str2coordinates(frame0['elbow']))
        joints.append(self.str2coordinates(frame0['wrist']))
